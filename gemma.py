...
def generate_gemma_response(context, instruction, question, response_key):
    template = f"""
    ...
    return response_text.rsplit(response_key + ":", 1)[-1].strip()

def gemma_answer(context, question):
    instruction = """
    ...
    """
    return generate_gemma_response(context, instruction, question, "###answer")

def gemma_hint(context, question):
    instruction = """
    ...
    """
    return generate_gemma_response(context, instruction, question, "###hint")

def gemma_provide_feedback(context, question, user_answer):
    instruction = """
    ...
    """
    return generate_gemma_response(context, instruction, f"{question}\n###user_answer:{user_answer}", "###feedback")

def gemma_check_verdict(context, question, user_answer, feedback):
    instruction = """
    ...
    """
    return generate_gemma_response(context, f"{question}\n###user_answer:{user_answer}\n###feedback{feedback}", instruction, "###verdict")
...
def create_questions(context):
    ...
    response_text = response.choices[0].message.content 
    return response_text.rsplit("###question:", 1)[-1].strip().split("?")
...
def generate_questions_and_answers(context):
    ...
    for question in questions:
        answer = gemma_answer(context, question)  
